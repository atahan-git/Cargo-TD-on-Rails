...
public class Tier1GunModuleDirectController : MonoBehaviour, IDirectControllable
{
	...
	public void ActivateDirectControl() {
		...
		gatlinificationSlider.gameObject.SetActive(myActiveGun.isGigaGatling || myActiveGun.gatlinificator);
	...
	void ApplyBulletTypes() {
		...
		DirectControlMaster.s.bulletSpeed = myGuns[0].bulletPrefab.GetComponent<Projectile>().speed;
	}
	...
	void OnHit() {
		...
		if (isSniper) {
			...
			myActiveGun.sniperDamageMultiplier = 1+sniperMultiplier;
			sniperAmount.text = $"Smart Bullets:\n+{sniperMultiplier*100:F0}%";
		}
	}
